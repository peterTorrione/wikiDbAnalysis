...
while (not line == ""):
    ...
    if startPageText in line:
        ...
        findex.write(title + "::" + str(indexDict[indexFileName]) + "\n")
        findex.close()
        ...
        f2.write(result[0])
        indexDict[indexFileName]+=result[1]

        count+=1;
        if not (count % 500):
            print "DB File: " + dbFileName + "Article: " + title + ", #" + str(count)
        if not(count % 10000):
            time.sleep(0.2)
